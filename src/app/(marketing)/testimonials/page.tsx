...
export default async function TestimonialsPage() {
  ...
  const getColorClasses = (color: string) => {
    const colorClasses: Record<string, { light: string, medium: string, dark: string, text: string }> = {
      blue: { 
        light: 'bg-blue-50', 
        medium: 'bg-blue-100', 
        dark: 'bg-blue-600',
      ...
      <div className="py-24 bg-gradient-to-b from-white to-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                return (
                  <div 
                    ...
                    className={`rounded-2xl p-8 shadow-lg transition-all duration-500 hover:shadow-xl hover:-translate-y-2 opacity-0 animate-fade-up ${
                      testimonial.featured ? colorClasses.light : 'bg-white'
                    }`}
                    style={{ animationDelay: `${animationDelay}ms` }}
                  >
                    <div className="relative mb-6">
                      <Quote className={`absolute -top-4 -left-4 h-8 w-8 opacity-20 ${colorClasses.text}`} />
                    </div>
                    
                    <blockquote className="text-slate-700 italic mb-6 relative z-10">
                    ...
                    <div className="flex justify-between items-end">
                      <div className="flex items-center">
                        {testimonial.imageUrl ? (
                          ...
                          <div className={`w-12 h-12 rounded-full flex items-center justify-center mr-4 shadow-md ${colorClasses.dark} text-white`}>
          ...
          <div className="mt-16 text-center opacity-0 animate-fade-in" style={{ animationDelay: '800ms' }}>
            ...
            <Button 
              size="lg" 
              className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-6 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
              asChild
            >
